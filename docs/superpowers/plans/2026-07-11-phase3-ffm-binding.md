...
- [ ] **Step 1: Java oracle artifacts — ALREADY INSTALLED (verified by the controller).** `org.gbif:name-parser-api:4.2.0-SNAPSHOT` and `org.gbif:name-parser:4.2.0-SNAPSHOT` are present in `~/.m2` (built from `/Users/markus/code/gbif/name-parser/` HEAD, `4.2.0-SNAPSHOT`, on JDK 25). **Depend on `4.2.0-SNAPSHOT` directly — do NOT re-run `mvn install` in the production repo** (it is unnecessary and building that repo may hit a permission prompt). Only if Maven reports the artifact unresolved, ask the controller to re-install; do not `cd` into the production repo yourself. Confirm presence read-only if you wish: `ls ~/.m2/repository/org/gbif/name-parser-api/4.2.0-SNAPSHOT/*.jar`.
