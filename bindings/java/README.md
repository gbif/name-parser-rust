...
(the released `5.0.1` interface) by downcalling the `nameparser-ffi` Rust cdylib
...
released `org.gbif:name-parser-api:5.0.1` artifact, resolved from repository.gbif.org (cached
...
- **JDK 22 or newer** to compile and run (`java.lang.foreign` is finalized in 22). Point
  ...
  ```
- `org.gbif:name-parser-api:5.0.1` — resolved automatically from repository.gbif.org (the
